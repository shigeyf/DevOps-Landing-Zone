...
### 5.1 役割: プラットフォーム ブートストラップ（Layer 1）

ランディングゾーンは **組織プラットフォーム ブートストラップ** として機能する。Layer 0 (`bootstrap`) の後に適用される最初の Terraform レイヤーであり、プロジェクトが依存するすべての共有 Azure リソースと VCS リソースを作成する。

運用上:

- **Layer 0** (`bootstrap`) は一度適用され、ほとんど更新されない。Layer 1 Storage Account を作成する。
- **Layer 1** (`devops-org-lz`) は、組織のプラットフォーム構成が変更された際に適用される（例: 新しいサブネット、新しい DevBox 定義、ガバナンスポリシーの変更）。
- 両レイヤーとも、異なる状態キーの下で同じ Layer 1 Storage Account に状態を格納する。
...
| -------------------------------------------------- | ---------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **ID リソースグループ**                            | プラットフォーム | **プロジェクト（Layer 2）** ✅ | 当初は組織レベルの共有コンテナーとして設計。改訂決定: プロジェクト UAMI は**プロジェクト RG**（Layer 2）に直接配置する。これにより設計が簡素化される — Layer 2 が Layer 1 リソースグループへの書き込みアクセスを必要とせず、各プロジェクトが自己完結的になり（プロジェクト RG を削除すれば全 ID もクリーンに削除）、レイヤー分離が保持される。                                                                                              |
...
```text
プラットフォーム LZ (Layer 1)             プロジェクト (Layer 2)
┌───────────────────────────┐           ┌──────────────────────────────────┐
...
当初の設計ではプロジェクト UAMI を組織レベルの共有 Identity RG に配置していた。これは**改訂**され、プロジェクト UAMI は各プロジェクト自身のリソースグループ（Layer 2）に配置する:

- **レイヤー分離:** Layer 2 が Layer 1 リソースグループへの書き込みアクセスを必要としない。各レイヤーが自己完結的。
- **プロジェクトライフサイクル:** プロジェクト RG を削除すれば全プロジェクト所有 ID もクリーンに削除される。共有 RG 内の孤立 UAMI が発生しない。
- **簡素化:** Layer 1 の RG が 1 つ減少（4 → 3）。プロジェクトが全リソースを単一 RG で管理。
- **発見容易性は維持:** Azure Resource Graph でタグや命名規則を用いて全プロジェクト RG から UAMI を横断クエリ可能。
- **RBAC に影響なし:** 各 UAMI はホスト RG に関係なくターゲットサブスクリプション上に個別のロール割り当てを持つ。

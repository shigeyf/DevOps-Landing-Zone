...
| --------------------------------- | ---------------------------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Identity RG**                   | Platform (org)               | **Project (Layer 2)** ✅ | Originally designed as a shared org-level container for project UAMIs. Revised decision: project UAMIs are now placed directly in the **Project RG** (Layer 2). This simplifies the design — Layer 2 does not need write access to any Layer 1 resource group, each project is self-contained (deleting the Project RG cleanly removes all identities), and layer separation is preserved.                                                                     |
...
#### Identity RG rationale (revised)

The original design placed project UAMIs in a shared org-level Identity RG. This has been **revised** — project UAMIs now reside in each project's own Resource Group (Layer 2). The reasons for the change:

- **Layer separation:** Layer 2 no longer needs write access to any Layer 1 resource group. Each layer is self-contained.
- **Project lifecycle:** Deleting the Project RG cleanly removes all project-owned identities. No orphaned UAMIs in a shared RG.
- **Simplicity:** One fewer RG at Layer 1 (3 RGs instead of 4). Projects own all their resources in a single RG.
- **Discoverability is preserved:** Azure Resource Graph can query UAMIs across all project RGs by tag or naming convention.
- **RBAC is unaffected:** Each UAMI has individual role assignments on target subscriptions regardless of which RG hosts it.

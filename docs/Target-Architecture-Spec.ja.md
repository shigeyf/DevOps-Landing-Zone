...
| **環境**         | 環境 → サブスクリプション対応   | サブスクリプションがプロジェクト時にマッピングされ、4 環境すべてが前提             | サブセット環境（例: dev + prod のみ）未サポート。プロジェクトは 4 つのサブスクリプションすべてを提供する必要がある。                                                                                               |
...
9. **サブセット環境サポート（アーキテクチャフォローアップ）:**
   - 現行の `project_github` はすべての環境が `subscriptions` に含まれていることを前提としている。ワークフローモジュールは 4 つすべてのティア（features、dev、staging、prod）の環境を生成する。
   - **アクション:** `subscriptions` が環境のサブセットのみを含むことを許可し、対応する GitHub Actions Environments、UAMI、ブランチ、フェデレーション ID 資格情報のみを生成する。

...
| #   | 関心事                                  | ルール                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **プライベート DNS 解決**               | ランナーが通信するすべてのプライベートエンドポイント（ブートストラップ SA、ブートストラップ KV、Layer 2 プロジェクト SA、ターゲットサブスクリプションのプライベートリンクサービス）は、プライベート DNS で解決される必要がある。BYO VNet は Platform LZ の `privatelink.*` DNS ゾーンに**リンクされなければならない**。プロジェクトモジュールは `devops_network.private_dns_zone_ids` で公開される DNS ゾーン ID を用い、`azurerm_private_dns_zone_virtual_network_link`（ゾーンごとに 1 リンク）で接続を確立する。`byo_vnet.link_to_platform_private_dns` フラグで切り替え可能。 |
| 2   | **ブートストラップ SA / KV への到達性** | （BYO VNet 内の）ランナーは、初期化時にブートストラップ Storage Account から tfstate を取得し、ブートストラップ Key Vault からシークレットを読み取る必要がある。両者は Platform LZ VNet にプライベートエンドポイントを持つ。BYO VNet は Platform LZ VNet と**ピアリング**されなければならない — あるいは既に接続性を提供しているハブ & スポークトポロジ経由で解決されなければならない。ピアリングは本 LZ の責務ではなく、企業のネットワークチームの責務である。                                                                                                                   |
| 3   | **ACR（ランナーイメージ）**             | ランナーはコンテナイメージを Platform LZ の ACR から Pull する必要がある。ACR はプライベートエンドポイント経由で BYO VNet から到達可能でなければならない — (2) と同じピアリング + DNS リンクで担保する。コンテナー実行 UAMI（プラットフォームスコープ）は、ACA Environment がどの VNet にあるかにかかわらず、ACA Job の Pull ID としてバインドされる。                                                                                                                                                                                                                            |
...
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **ブートストラップ Storage Account / Key Vault の Private Endpoint** | ブートストラップ SA（Layer 1 tfstate）と Key Vault は **組織スコープの単一リソース**（§1 の Table A）。それらの PE は LZ apply 時に Platform LZ VNet 内へ一度だけデプロイされる。当該 VNet 内のすべてのプロジェクトサブネットは **追加設定なし** で到達可能。                             | 各プロジェクト VNet は当該 PE へ到達するために自前のピアリングが必要、または PE 一式をプロジェクト VNet 内に複製する必要がある — しかし基盤の SA/KV は組織 singleton なので複製はできない。結果として「プロジェクト ↔ Platform VNet ピアリング」を強制され、これは §8.0.1 の BYO 整合性ルール作業そのもの。 |
| **Private DNS ゾーン (`privatelink.*`)**                             | 特定の `privatelink.<service>.core.windows.net` ゾーンは LZ サブスクリプション内で実質 **singleton**。Platform LZ が正規ゾーンを所有し、LZ VNet がそれらに一度だけリンクされる。SA/KV/ACR/ACA の PE が公開する A レコードは同 VNet 内のどのプロジェクトサブネットからも正しく解決できる。 | 各プロジェクト VNet は (a) 同じ `privatelink` ゾーンを自前で複製する（split-horizon 衝突と A レコード重複を招く）か、(b) ゾーン × プロジェクト VNet 毎に明示的な `privateDnsZoneVirtualNetworkLink` を作成するかが必要 — つまり §8.0.1 ルール #1 を全プロジェクトに無償で支払わせることになる。              |
...
**2 行要約。** `platform` モードにおいて VNet を Platform レイヤーで所有することは、**Platform LZ がすでに ブートストラップ SA/KV の PE、共有 Private DNS ゾーン、共有 NAT、共有 ACR、および共有アドレス計画ガバナンスを所有している** という事実から直接導かれる。ランナーサブネットを同じ VNet 内に同居させることが、これら組織共有リソースへのアクセスをプロジェクトに提供する **最も安価・単純・正しい** 方法であり、§8.0.1 の 7 つの整合性ルールをプロジェクト毎に再実装する必要を排除する。プロジェクトに別所に居る実需（エンタープライズスポーク、中央ファイアウォール、オンプレミスピアリング等）が生じたら `byo` モードへ切り替え、これらのルールを明示的に再構築するコストを受け入れる。中間に有用な第三の選択肢は存在しない。

#### 8.0.5 詳細ネットワーク図（platform モードと BYO モード）

§8.0 の 3 層モデル図は可読性のため、サブネット単位、DNS リンク、ピアリング、ID バインディングなどの詳細を意図的に省略している。以下の図は 2 つの運用モードについてそれらの詳細を補い、ランナーコンテナから ブートストラップ SA/KV PE、ACR、Layer 2 プロジェクト tfstate SA、ターゲットサブスクリプションへ至るランタイムネットワーク経路を曖昧さなく示す。また、どの矢印が `devops/lz` によって作成され、どれがプロジェクトモジュールにより作成され、どれが本 LZ の対象外（エンタープライズの hub-and-spoke）かも明示する。

##### 8.0.5.1 `network_mode = "platform"` — ランナーは共有 Platform LZ VNet 内に同居

`platform` モードでは、プロジェクトの CI/CD コンピュートは共有 Platform LZ VNet の **プロジェクト専用サブネットスライス** に着地する。ブートストラップ SA/KV/ACR の Private Endpoint、`privatelink.*` DNS ゾーン、NAT エグレスはすべて同じ VNet 内にあるため、ランナーは **ピアリングなし、プロジェクト毎の DNS リンクなし** でそれらに到達する — §8.0.1 の 7 つの整合性ルールが構造的に充足される（§8.0.4）。
...
Platform LZ サブスクリプション内の Private DNS ゾーン（devops/lz により VNet リンクは各 1 本）:
  privatelink.blob.core.windows.net           ← ブートストラップ SA、プロジェクト Layer 2 SA レコード
  privatelink.vaultcore.azure.net             ← ブートストラップ KV レコード
  privatelink.azurecr.io                      ← ACR レコード
  privatelink.<region>.azurecontainerapps.io  ← ACA Environment レコード
                              │
                              └── Platform LZ VNet に一度リンク（全プロジェクトサブネットを覆う）
                                  → ランナーはブートストラップ SA/KV/ACR/Layer 2 SA を Private IP で解決
...
- **PE 到達性が自動。** ブートストラップ SA、ブートストラップ KV、ACR、Layer 2 プロジェクト SA の PE がランナーサブネットと同じ VNet 内にあるため、Private IP で到達するのにピアリングは不要（§8.0.4 「ブートストラップ SA/KV PE」行、§8.0.1 ルール #2）。
- **Private DNS 解決が自動。** 各 `privatelink.*` ゾーンは `devops/lz` により Platform LZ VNet に **一度だけ** リンクされる。ランナーサブネットはこのリンクを自動的に継承する（§8.0.1 ルール #1、§8.0.4 「Private DNS ゾーン」行）。
...
`byo` モードでは、ランナーサブネットは LZ が作成・所有しない **事前プロビジョニング済みのエンタープライズスポーク VNet** に存在する。§8.0.1 の 7 つの整合性ルールは明示的に成立させる必要がある: ブートストラップ SA/KV/ACR の PE に到達するため BYO VNet と Platform LZ VNet 間のピアリング（またはエンタープライズハブ経由のトランジット）が必要であり、Private Link ホスト名が Platform PE の IP に解決されるよう BYO VNet を Platform LZ の `privatelink.*` DNS ゾーンへリンクする必要がある。
...
BYO VNet 内ランナーの Private DNS 解決経路（§8.0.1 ルール #1）:
  ランナー → "<sa-name>.blob.core.windows.net" の DNS クエリ
        ...
        → A レコードは Platform LZ VNet 内 PE IP（10.10.1.x）を指す
...
| -------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Platform LZ VNet、NAT、ブートストラップ SA/KV/ACR Private Endpoint、`privatelink.*` DNS ゾーン     | ✅ 作成・所有（常時 — `platform` モードと同一）。                                                                                   | —                                                                                                                                                                   | —                                                                                             |
...
| BYO VNet ↔ Platform LZ VNet ピアリング（ルール #2 ブートストラップ SA/KV PE 到達、ルール #3 ACR） | —                                                                                                                                   | —                                                                                                                                                                   | ✅ hub-and-spoke または直接ピアリング。本 LZ は apply 時に到達性を検証するのみ。              |
...
| Layer 2 プロジェクト tfstate Storage Account + その PE を BYO PE サブネットに（§1 表 C）           | —                                                                                                                                   | ✅（ターゲット — Remaining Issue #7）。現状は Layer 1 ブートストラップ SA にコンテナを作成。                                                                        | —                                                                                             |
| ランナー VNet（BYO）からターゲットサブスクリプションの Application/Workload VNet への接続          | —                                                                                                                                   | —                                                                                                                                                                   | ✅ エンタープライズハブ経由の標準 hub-and-spoke ピアリングまたは VPN/ER（§8.0.2 を参照）。    |

§8.0.1 のピアリングまたは DNS リンクが欠落している場合、ランナーは決定論的かつ可観測なエラーで失敗する（`privatelink.*` ホスト名の DNS NXDOMAIN、ブートストラップ SA/KV PE IP への TCP タイムアウト、ACA Job init コンテナでの ACR pull 失敗）。プロジェクトモジュールはこれらを可能な限り Terraform precondition として表面化し（ルール #4 のサブネット delegation）、それ以外はランタイム失敗として現れる — 公開エンドポイントへサイレントにフォールバックすることはない。

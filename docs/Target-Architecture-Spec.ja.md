...
| **組織**         | ~~ID リソースグループ~~         | ~~プラットフォーム LZ で空の RG が作成される~~                                                                       | 廃止 — プロジェクト UAMI はプロジェクトスコープ RG（Layer 2）に配置。レイヤー分離の簡素化: Layer 2 が Layer 1 リソースグループへの書き込みアクセスを必要としない。テーブル C 参照。                                                                                                                  |
...
| **環境**         | ID 戦略                         | UAMI が環境 × ジョブタイプごとに作成される                                                                           | 戦略は [ADR-003](./adr/ADR-003-project-multi-repo-model.ja.md) で明確にドキュメント化された（UAMI はプロジェクトスコープで、プロジェクト RG 内に Layer 2 で作成される。LZ にはグローバルなサブスクリプションレジストリは存在しない）。リポジトリ別 UAMI 分離はターゲット（上記リポジトリ行を参照）。 |
...
- 各プロジェクトはプロジェクト作成時に **7 つの UAMI** を取得: `feat-plan`、`dev-plan`、`stg-plan`、`prod-plan`、`dev-apply`、`stg-apply`、`prod-apply`。全 UAMI は**プロジェクト RG**（Layer 2）に置かれ、プロジェクト自己完結的に管理される。プロジェクト RG を削除すれば全 ID もクリーンに削除される。
...
│  ┌─────────────────────────┐  ┌──────────────────────────────────────┐ │   │
│  │ Identity (Project RG内) │  │ Layer 2 Storage Account              │ │   │
...
| カテゴリ      | リソース                                                                                           | 何か                                                                                                                               | 何のため                                                                                                                                                                                                                         |
| ------------- | -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ID            | 7 個のプロジェクト UAMI                                                                            | プロジェクトスコープ RG に作成される UAMI                                                                                          | 環境別 × ジョブ種別の ID: `feat-plan`, `dev-plan`, `stg-plan`, `prod-plan`, `dev-apply`, `stg-apply`, `prod-apply` — 最小権限ワークフロー用に (env × job) ごとに 1 ID（[ADR-003](./adr/ADR-003-project-multi-repo-model.ja.md)） |
...
| RG            | Layer 2 プロジェクトリソースグループ                                                               | プラットフォームサブスクリプション内のプロジェクトスコープ RG                                                                      | Layer 2 Storage Account、プロジェクト Key Vault、プロジェクト UAMI を格納するリソースグループ（全プロジェクトリソースを一元管理）                                                                                                |
...
│  terraform apply                                                │
│  作成するもの:                                                   │
│    • エージェント RG + ACR + Log Analytics + コンテナー実行用 UAMI│
│    • ネットワーク RG + VNet + サブネット + DNS ゾーン + NAT GW    │
│    • DevBox RG + Dev Center + 定義                               │
│    • Bootstrap KV シークレット (VCS PAT)                         │
│    • VCS ガバナンス                                                │
│  出力:                                                           │
...
| #   | 質問                                                                                                                              | 選択肢                                                                | 推奨事項                                                                                                                                                                                                                                                                                                                                                                                               |
| --- | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | GitOps ガバナンスリポジトリは Platform LZ（Layer 1）の一部として作成すべきか、独立して設定すべきか？                              | LZ の一部 / 独立 / テンプレートリポジトリ                             | **✅ 決定:** 独立した **テンプレートリポジトリ** として設定する。ガバナンスリポジトリは **Project LZ リポジトリ** を Git サブモジュールとして参照する（DevOps Landing Zone リポジトリの `infra/devops-project-lz/` と同じサブモジュール）。これにより、直接 apply パスと GitOps 駆動オンボーディングパスの両方で、プロジェクトプロビジョニングコードの唯一の信頼できるソースが保証される。             |

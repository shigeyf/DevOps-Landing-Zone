...
| 領域                                | 現在の状態                                                                            | ギャップ                                                                                                                                                                             |
| ----------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **ブートストラップ**                | 単一のブートストラップが tfstate 用の Storage Account と Key Vault を作成             | ブートストラップは Layer 1（プラットフォーム）状態ストレージのみを作成しており、プロジェクトチーム独自の IaC 用のプロジェクトごとの Layer 2 状態ストレージがプロビジョニングされない |
...
2. **2 層状態管理** モデルを導入する：Layer 1 はプラットフォーム/DevOps LZ 状態（ブートストラップ、LZ、プロジェクトプロビジョニング）用、Layer 2 はプロジェクトごとのアプリケーション IaC 状態（プロジェクトプロビジョニング時に作成）用。
...
| **プラットフォーム ランディングゾーン** | 組織ごとに一度プロビジョニングされる共有インフラストラクチャレイヤー (`devops/lz`)。Azure リソースグループ（ID、エージェント、ネットワーク、DevBox）と VCS ガバナンスリソースを作成する。組織スコープリソースには ACR、Log Analytics、コンテナー実行 UAMI、Dev Center、プラットフォーム VNet、ガバナンスルールセットが含まれる。自身の tfstate はブートストラップの Storage Account に格納される。ランナー用の ACA Environment はプラットフォームレベルではなく、プロジェクトレベルで作成される。 |
...
│    • ACA Environment（プラットフォーム VNet または BYO VNet 内）  │
│    • ランナー（ACA ジョブまたは ACI）                             │
│    • DevBox プロジェクトプール                                     │
│    • Layer 2 Storage Account（プロジェクト独自の IaC 状態用）     │
│                                                                 │
...
| **VCS ガバナンスリソース**                         | プラットフォーム | **プラットフォーム** ✅ | 組織レベルのルールセット（GitHub）とエージェントプール（Azure DevOps）。組織スコープで正しい。                                                                                                                                                                                                                                           |
...
- コンテナーイメージのビルドタスク
- VCS ガバナンスリソース（組織レベルのルールセット、ランナーグループ、エージェントプール）

### 5.4 変更点

#### 5.4.1 ACA Environment のプロジェクトレベルへの移動（変更）

ACA Environment はプラットフォーム LZ では作成されなくなる。代わりに、各プロジェクトモジュールが適切な VNet 内に独自の ACA Environment を作成する。詳細な根拠はセクション 5.2 を参照。

...
| **組織**         | ACA Environment スコープ        | ~~ACA Environment がプラットフォーム LZ で作成~~ → **プロジェクトレベルに移動済み（§14#6 実装完了）**                | ACA Environment はプロジェクトスコープのリソースとして実装済み。各プロジェクトが自身のネットワークコンテキストに ACA Environment を作成する。§5.4.1 参照。                                                                                                                                                                                                                 |
...
| Agents          | ~~ACA Environment~~ _(プロジェクトレベルに移動済み — §5.4.1 / §14#6 実装完了)_ | _(LZ レベルでは作成しない)_                                                | ACA Environment はプロジェクトスコープのリソースに移行済み。LZ はプロジェクトレベルの ACA Environment が利用する共有インフラ（ACR、Log Analytics、container-run UAMI）を引き続き提供。Table C のプロジェクトスコープ ACA Environment 行を参照。 |
...
| Compute       | ACA Environment _(§5.4.1 / §14 #6 — 実装完了)_                | プロジェクトスコープの Azure Container Apps Environment                           | プロジェクトの self-hosted runner Job を実行。プロジェクトのランナーネットワークコンテキスト（`platform` モードでは Platform LZ VNet 内のプロジェクト専用サブネット、`byo` モードでは BYO VNet）にバインド |
...
| **プラットフォーム ランディングゾーン** | 組織ごとに一度プロビジョニングされる共有インフラストラクチャレイヤー (`devops/lz`)。Azure リソースグループ（ID、エージェント、ネットワーク、DevBox）と共有コンピューティング/レジストリリソースを作成する。組織スコープリソースに**現在実装されている**ものは: ACR、Log Analytics、コンテナー実行 UAMI、Dev Center、プラットフォーム VNet、プライベート DNS ゾーン。VCS ガバナンスリソース（組織レベルのルールセット、ランナーグループ）はセクション 9 で**提案**されているが未実装。ACA Environment は**プロジェクトレベルに移動済み**（§5.4.1 / §14#6 — 実装完了）。自身の tfstate はブートストラップの Storage Account に格納される。 |
...
│    • ランナー（プロジェクト ACA Env を使用した ACA ジョブまたは ACI）│
│    • Layer 1 SA 内の Blob コンテナー（プロジェクト tfstate + ログ）│
│    • DevBox プロジェクトプール                                     │
│    • ACA Environment（プロジェクトごと、§5.4.1 / §14#6 — 実装済み）│
│  作成（ターゲット — コード変更待ち）:                                │
...
- **ACA Environment はプロジェクトスコープ（実装済み）。** §5.4.1 のとおり、ACA Environment はプロジェクトモジュールがプロジェクトのランナーサブネット内に作成する。Platform LZ は共有依存（ACR、Log Analytics、container-run UAMI）のみを提供する。コードリファクタリングは完了（§14 #6）。
...
6. **ACA Environment のリファクタリング — プラットフォーム LZ からプロジェクトレベルへの移動 ~~（アーキテクチャフォローアップ）~~ 完了:**
   - **ターゲットアーキテクチャはセクション 5.4.1 およびセクション 8.0 で確立済み。** ACA Environment はターゲット設計ではプロジェクトレベルのリソースであり、プロジェクトの DevOps ネットワークコンテキスト（`network_mode = "platform"` ではプラットフォーム VNet、`network_mode = "byo"` では BYO VNet）内にプロジェクトモジュールが作成する。
   - **コード変更実施済み:**
     - (a) `devops/lz/agents.container_app_env.tf` から ACA Environment モジュール（`module "aca"`）と `container_app_environment_id` / `container_app_workload_profile_name` 出力を削除。ファイルにはマイグレーションコメントのみ残存。
     - (b) `project_github/agents.container_app_env.tf` を追加 — プロジェクトの有効な ACA サブネットを使用してプロジェクトスコープの ACA Environment を作成。`platform` モードではプロジェクト専用の ACA サブネット（`azurerm_subnet.aca`）をプラットフォーム LZ VNet 内に作成、`byo` モードでは外部提供の `byo_aca_subnet_id` を使用。
     - (c) `project_github/_variables.network.tf` を追加 — 新変数: `network_mode`（`"platform"` | `"byo"`）、`platform_aca_subnet_address_prefix`、`byo_aca_subnet_id`、`enable_aca_zone_redundancy`。
     - (d) `project_github/_locals.devops.tf` を更新 — `container_app_environment_id` が LZ remote state 出力ではなくプロジェクトレベルの `module.aca` から取得されるように変更。`log_analytics_workspace_id`（`log_analytics_id` からリネーム）を追加。
     - (e) LZ `devops_agents` 出力を再構成: `container_run_uami_client_id` と `log_analytics_workspace_id`（リネーム）をエクスポート。ACA 固有フィールドは削除。
     - (f) LZ `devops_network` 出力に `vnet_id` と `vnet_name` を追加（プラットフォームモードでのサブネット作成のためにプロジェクトモジュールが使用）。
   - **既存デプロイ向けマイグレーション:** LZ state に対して `terraform state rm 'module.aca[0]'` を実行後、プロジェクトモジュールを再適用して ACA Environment をプロジェクトレベルで再作成する。

...
| **環境**         | ID 戦略                         | UAMI が環境 × ジョブタイプごとに作成される                                                                           | 戦略は [ADR-003](./adr/ADR-003-project-multi-repo-model.ja.md) で明確にドキュメント化された（UAMI はプロジェクトスコープで、プロジェクト RG 内に Tier 2 で作成される。LZ にはグローバルなサブスクリプションレジストリは存在しない）。リポジトリ別 UAMI 分離はターゲット（上記リポジトリ行を参照）。 |

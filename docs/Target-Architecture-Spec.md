...
| Agents     | ACA Environment _(current — to be moved per §5.4.1)_   | Azure Container Apps Environment bound to the platform VNet runner subnet        | Today: hosts self-hosted runner Jobs at LZ scope. Target: moved to the project module so it binds to the **project's runner network context** (project-dedicated subnet slice in `platform` mode, BYO VNet in `byo` mode) |
...
| Compute      | ACA Environment _(target — §5.4.1, §14 #6)_                  | Project-scoped Azure Container Apps Environment                             | Runs the project's self-hosted runner Jobs. Bound to the project's runner network context (project-dedicated subnet in the Platform LZ VNet for `platform` mode, BYO VNet for `byo` mode)              |
...
│    • DevBox project pool                                        │
│  Creates (target — code change pending, see §5.4.1 / §14#6):    │
...
| **ACA Environment**               | Platform (org) | **Project** ❌        | **Issue:** The ACA Environment is currently created at the platform level and bound to the platform VNet. This is architecturally incorrect because (a) an ACA Environment is bound to exactly **one** VNet and so cannot serve projects that bring their own VNet, and (b) self-hosted runners must operate in the **project's DevOps VNet** to have network line-of-sight to the resources they deploy (private endpoints, peered application VNets, on-prem routes). **Target:** The ACA Environment is a **project-level** resource (Section 5.4.1). The Platform LZ provides only shared infrastructure (ACR, Log Analytics, container-run UAMI, private DNS zones) consumed by project-level ACA Environments. For `network_mode = "platform"`, the project's ACA Environment is created in the platform VNet's ACA subnet; for `network_mode = "byo"`, it is created in the BYO VNet's ACA subnet. |
...
| #   | Concern                             | Rule                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Private DNS resolution**          | All private endpoints the runner contacts (bootstrap SA, bootstrap KV, Layer 2 project SA, target subscription private-link services) must resolve via private DNS. The BYO VNet **must be linked** to the Platform LZ's `privatelink.*` DNS zones. The project module performs this linkage via `azurerm_private_dns_zone_virtual_network_link` (one link per zone) using the DNS zone IDs published in `devops_network.private_dns_zone_ids`. The `byo_vnet.link_to_platform_private_dns` flag toggles this.  |
| 2   | **Bootstrap SA / KV reachability**  | The runner (in the BYO VNet) needs to pull tfstate from the bootstrap Storage Account and read secrets from the bootstrap Key Vault during initialization. Both have private endpoints in the Platform LZ VNet. The BYO VNet **must be peered** to the Platform LZ VNet — or alternatively resolve those endpoints through a hub-and-spoke topology that already provides connectivity — so the runner can reach the private IPs. Peering is the responsibility of the enterprise networking team, not this LZ. |
| 3   | **ACR (runner images)**             | The runner must pull its container image from ACR (in the Platform LZ). The ACR must be reachable via private endpoint from the BYO VNet — covered by the same peering + DNS linkage as (2). The container-run UAMI (platform-scoped) is bound as the pull identity of the ACA Job regardless of which VNet the ACA Environment is in.                                                                                                                                                                          |
...
6. **ACA Environment refactoring — move from Platform LZ to project level (architecture follow-up):**
   - **Target architecture established in Section 5.4.1 and Section 8.0.** The ACA Environment is a project-level resource in the target design; it is created by the project module inside the project's DevOps VNet (platform VNet for `network_mode = "platform"`, BYO VNet for `network_mode = "byo"`). The code change is still pending.
   - **Current state:** The ACA Environment is created in `devops/lz/agents.container_app_env.tf` and consumed by `project_github` via the `container_app_environment_id` remote-state output. This binds all projects to the single platform VNet and structurally blocks `network_mode = "byo"`.
   - **Action (code):** (a) Remove `agents.container_app_env.tf` and the `container_app_environment_id` output from `devops/lz`; (b) add ACA Environment creation in `project_github` using the project's effective ACA subnet (platform or BYO) resolved as described in Section 8.4; (c) update runner job (ACA Job) creation in the project module to bind to the new per-project environment; (d) document the `terraform state rm` migration step for existing deployments.

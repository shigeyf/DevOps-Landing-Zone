...
| **Organization**  | ~~Identity RG~~            | ~~Empty RG created at Platform LZ~~                                                                                     | Removed — project UAMIs now reside in the project-scoped RG (Layer 2). This simplifies layer separation: Layer 2 no longer needs write access to any Layer 1 resource group. See Table C.                                                                          |
...
| **Environment**   | Identity strategy          | UAMIs created per environment × job type                                                                                | Strategy now documented in [ADR-003](./adr/ADR-003-project-multi-repo-model.md) (UAMIs are project-scoped, created at Layer 2 in the Project RG; no global subscription registry at the LZ). Per-repo UAMI isolation is still a target (see Repository row above). |
...
- Each project gets **7 UAMIs** created at project time: `feat-plan`, `dev-plan`, `stg-plan`, `prod-plan`, `dev-apply`, `stg-apply`, `prod-apply`. All UAMIs live in the **Project RG** (Layer 2) and are project-scoped in naming and lifecycle. This keeps each project self-contained — deleting the Project RG cleanly removes all project-owned identities.
...
│  └───────────────────────────────────────────────────────────────────────┘ │
│                                  │                                          │
│  ┌─ Platform LZ (devops-org-lz) ──────▼──── Layer 1 ─────────────────────┐ │
...
│        ┌─────────────────────────┴────────────────────────┐                 │
│        ▼                                                  ▼                 │
│  ┌─ PROJECT A (devops-project-lz) ── Layer 2 ┐   ┌─ PROJECT B ────────────┐│
...
│        ┌─────────────┴──────────────────┐          ┌───────────┴──────┐     │
│        ▼                                ▼          ▼                  ▼     │
│  ┌─ REPO: repo-infra ── Layer 3 ──┐ ┌─ REPO: repo-app ─┐  ┌─ REPO ────────┐│
...
│    UAMI id, Platform VNet/subnet ids, DNS zone ids                       │  │
│  • project YAML: name, repos[], subscriptions{}, network_mode, byo_vnet  │  │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  Project-scoped Azure resources ────────────────────────────────────────┐   │
│  ┌─────────────────────────┐  ┌──────────────────────────────────────┐ │   │
│  │ Identity (in Project RG)│  │ Layer 2 Storage Account              │ │   │
...
| Category     | Resource                                                                                 | What is it                                                                                                                   | What is it for                                                                                                                                                                                                                                         |
| ------------ | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Identity     | 7 Project UAMIs                                                                          | UAMIs created in the project-scoped Resource Group                                                                           | Per-environment, per-job-type identities: `feat-plan`, `dev-plan`, `stg-plan`, `prod-plan`, `dev-apply`, `stg-apply`, `prod-apply` — one identity per (env × job) for least-privilege workflows ([ADR-003](./adr/ADR-003-project-multi-repo-model.md)) |
...
| State        | Layer 2 Project Resource Group                                                           | New project-scoped Resource Group inside the platform subscription                                                           | Houses the Layer 2 Storage Account, Project Key Vault, Layer 2 Private Endpoint, and project UAMIs — all project-owned resources in a single RG                                                                                                        |
...
│  │    – DevBox RG (Dev Center + definitions)                   │  │
...
│  Outputs:                                                       │
│    • devops_agents, devops_network, devops_devbox,              │
│      container_specs, options                                   │
│                                                                 │

...
| **Organization**  | ACA Environment scoping    | ~~ACA Environment created at Platform LZ~~ → **Moved to project level (§14#6 implemented)**                             | ACA Environment is now project-scoped. Each project creates its own ACA Environment in its runner network context. See §5.4.1.                                                                                                                                                                                                                                                                    |
...
| Agents     | ~~ACA Environment~~ _(moved to project level — §5.4.1 / §14#6 implemented)_ | _(no longer created at LZ level)_                                                | The ACA Environment is now a project-scoped resource. The LZ continues to host shared infra (ACR, Log Analytics, container-run UAMI) consumed by project-level ACA Environments. See Table C for the project-scoped ACA Environment row. |
...
Provisions one project's Azure resources, identities, and VCS-side configuration. Run once per project. Consumes Platform LZ outputs (Table B) via `terraform_remote_state`. The same resource set is created by `project_azuredevops` (target — §14 #8) so GitHub and Azure DevOps projects are functionally equivalent.
...
| State        | Layer 2 Storage Account _(target — §14 #7)_                  | Per-project Storage Account (planned)                                       | Stores **Layer 2** tfstate for the project team's own application IaC (today the project's blob containers live in the Layer 1 SA)                                                                     |
| Secrets      | Project Key Vault _(target — §14 #7 / §8.0.1 rule #3)_       | Per-project Key Vault (planned, deployed in the project's runner VNet)      | Stores the project team's own secrets and keys (e.g., app config, DB passwords, signing keys). Distinct from the org-level bootstrap Key Vault (which only holds VCS PATs used at provisioning time).  |
| State        | Layer 2 Private Endpoint + DNS link _(target — §14 #7)_      | Private Endpoint for the Layer 2 SA in the project's runner network context | Keep app-tfstate access on the private network used by the project's runners                                                                                                                           |
| State        | Per-project tfstate blob containers (current)                | Blob containers in the Layer 1 SA                                           | Today's per-project tfstate location until Layer 2 is implemented (§14 #7)                                                                                                                             |
| Compute      | ACA Environment _(§5.4.1 / §14 #6 — implemented)_            | Project-scoped Azure Container Apps Environment                             | Runs the project's self-hosted runner Jobs. Bound to the project's runner network context (project-dedicated subnet in the Platform LZ VNet for `platform` mode, BYO VNet for `byo` mode)              |
...
| VCS — ADO    | Azure DevOps Project + repos + pipelines _(target — §14 #8)_ | Azure DevOps Project + Git repos + YAML pipelines                           | Functional equivalent of the GitHub stack above, so the abstraction in §7 holds end-to-end                                                                                                             |
...
│  │  * ACA Environment is now a project-level resource — §5.4.1 │  │
│  │    (currently still at LZ in code; refactor tracked as §14#6)│  │
...
│    • DevBox project pool                                        │
│    • ACA Environment per project (§5.4.1 / §14#6 — implemented)│
│  Creates (target — code change pending):                         │
...
> **Note:** The subsections below describe the **target architecture**. §5.4.1 is the formalized target for the ACA Environment refactor (v0.5); §5.4.2 and §5.4.3 are still proposed. None of these items are yet implemented in the codebase. See Section 14 (Remaining Issues) for the implementation status of each item.
...
The implementation is tracked as Section 14 item #6. Until the refactor is merged, the current code path (ACA Environment at LZ, consumed via `container_app_environment_id`) continues to work for `network_mode = "platform"` projects; BYO VNet projects cannot use it and are blocked on this item.
...
- **ACA Environment is project-scoped (implemented).** Per §5.4.1, the ACA Environment is created by the project module inside the project's runner subnet. Platform LZ provides only the shared dependencies (ACR, Log Analytics, container-run UAMI). The code refactor is complete (§14 #6).
...
| Layer 2 project tfstate Storage Account + project Key Vault + their PEs in the BYO PE subnet (Tab. C in §1, rule #3 — both project-owned, in the BYO VNet, no platform peering needed for these) | —                                                                                                                                      | ✅ (Target — Layer 2 SA tracked as Remaining Issue #7; project KV is created by the project module alongside the Layer 2 SA.) Currently project tfstate containers still live in the Layer 1 bootstrap SA. | —                                                                                                           |
...
6. **ACA Environment refactoring — move from Platform LZ to project level ~~(architecture follow-up)~~ DONE:**
   - **Target architecture established in Section 5.4.1 and Section 8.0.** The ACA Environment is a project-level resource in the target design; it is created by the project module inside the project's DevOps network context (platform VNet for `network_mode = "platform"`, BYO VNet for `network_mode = "byo"`).
   - **Code changes implemented:**
     - (a) Removed the ACA Environment module (`module "aca"`) from `devops/lz/agents.container_app_env.tf` and the `container_app_environment_id` / `container_app_workload_profile_name` outputs from the LZ. The file now contains only a migration comment.
     - (b) Added `project_github/agents.container_app_env.tf` — creates a project-scoped ACA Environment using the project's effective ACA subnet. In `platform` mode the project creates a dedicated ACA subnet (`azurerm_subnet.aca`) inside the Platform LZ VNet; in `byo` mode the project uses the externally-provided `byo_aca_subnet_id`.
     - (c) Added `project_github/_variables.network.tf` — new variables: `network_mode` (`"platform"` | `"byo"`), `platform_aca_subnet_address_prefix`, `byo_aca_subnet_id`, `enable_aca_zone_redundancy`.
     - (d) Updated `project_github/_locals.devops.tf` — the `container_app_environment_id` now comes from the project-level `module.aca` instead of the LZ remote-state output; added `log_analytics_workspace_id` (renamed from `log_analytics_id` for clarity).
     - (e) LZ `devops_agents` output restructured: exports `container_run_uami_client_id` and `log_analytics_workspace_id` (renamed); no longer exports ACA-specific fields.
     - (f) LZ `devops_network` output now includes `vnet_id` and `vnet_name` for use by the project module when creating subnets in platform mode.
   - **Migration for existing deployments:** Run `terraform state rm 'module.aca[0]'` against the LZ state, then re-apply the project module to recreate the ACA Environment at the project level.
   - The Platform LZ continues to provide shared infrastructure consumed by project-level ACA Environments: ACR (images), Log Analytics (logging), container-run UAMI (image pull + secret access), and private DNS zones (name resolution).

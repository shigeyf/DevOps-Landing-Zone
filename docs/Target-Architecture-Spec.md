...
| **Environment**   | Identity strategy          | UAMIs created per environment × job type                                                                                | Strategy now documented in [ADR-003](./adr/ADR-003-project-multi-repo-model.md) (UAMIs are project-scoped, created at Tier 2 in the Project RG; no global subscription registry at the LZ). Per-repo UAMI isolation is still a target (see Repository row above). |
